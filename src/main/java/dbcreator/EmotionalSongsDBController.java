...
public class EmotionalSongsDBController {
    ...
    public void creaDBEmotionalSongs(){
        ...
        if (!fileSQL.exists() || !fileCSV.exists()) {
            ...
            try {
                ...
                if (!checkEsisteDatabase(connection, nomeDatabase)) {
                    ...
                    Platform.runLater(() -> {
                        ...
                        showSuccessAlert("Contenuto dei file 'creaTabelle.sql' e 'canzoni.csv' caricati nel database con successo.");
                    });
                } else  showErrorAlert("Il database '" + nomeDatabase + "' esiste già. Non è necessario caricare il contenuto del file .sql.");
            } catch (IOException e) {
                e.printStackTrace();
